        ...
        for v in u.Neighbours:
            if v is not None:
                ...
                if visited[vposindex] is False:
                    ...
                    # New distance is the distance of the path from the start,
                    # through U, to V.
                    newdistance = distances[uposindex] + d

                    # A* includes a remaining cost, the f cost. In this case
                    # we use manhattan distance to calculate the distance from
                    # V to the end. We use manhattan again because
                    # A* works well when the g cost and f cost are balanced.
                    # https://en.wikipedia.org/wiki/Taxicab_geometry
                    remaining = abs(vpos[0] - endpos[0]) + abs(
                        vpos[1] - endpos[1])

                    # Notice that we don't include f cost in this first check.
                    # We want to know that the path *to* our node V
                    # is shortest
                    if newdistance < distances[vposindex]:
                        vnode = nodeindex[vposindex]

                        if vnode is None:
                            # V goes into the priority queue with a cost of
                            # g + f. So if it's moving closer to the end,
                            # it'll get higher priority than some other nodes.
                            # The order we visit nodes is a trade-off between
                            # a short path, and moving closer to the goal.
                            ...
                            # As above, we decrease the node since we've found
                            # a new path. But we include the f cost, the
                            # distance remaining.
